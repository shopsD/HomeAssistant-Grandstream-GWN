    ...
    async def _unpublish_all_data(self) -> None:
        ...
        for gwn_network in gwn_networks:
            ...
            for gwn_device in gwn_network.devices:
                try:
                    _LOGGER.info(f"Unpublishing old Device with MAC {gwn_device.mac}")
                    ...
                    _LOGGER.info(f"Unpublished old Device with MAC {gwn_device.mac}")
                except Exception as e:
                    _LOGGER.info(f"Failed to unpublish Device with MAC {gwn_device.mac}: {e}")
            for gwn_ssid in gwn_network.ssids:
                try:
                    _LOGGER.info(f"Unpublishing old SSID with ID {gwn_ssid.id}")
                    ...
                    _LOGGER.info(f"Unpublished old SSID with ID {gwn_ssid.id}")
                except Exception as e:
                    _LOGGER.info(f"Failed to unpublish SSID with ID {gwn_ssid.id}: {e}")
            try:
                _LOGGER.info(f"Unpublishing old Network with ID {gwn_network.id}")
                network_payload = self._serialise_network(gwn_network)
                await self._mqtt_client.unpublish_network(network_payload, True)
                _LOGGER.info(f"Unpublished old Network with ID {gwn_network.id}")
            except Exception as e:
                _LOGGER.info(f"Failed to unpublish Network with ID {gwn_network.id}: {e}")
